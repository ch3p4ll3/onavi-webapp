function reset_add_credit() {
    let modal = document.getElementById("modal-text");
    document.getElementById("action-title").innerText = "Add Credit";
    let modal_btn = document.getElementById("modal-save-btn")

    value = localStorage.getItem("reset_add");
    ...
    modal_btn.setAttribute("onclick", "reset_add_credit_send(this)")

    const myModalAlternative = new bootstrap.Modal('#action-modal', {});
    myModalAlternative.show();
}

async function reset_add_credit_send() {
    let credit_to_add = document.getElementById("modal-text").querySelector("input").value;

    localStorage.setItem("reset_add", credit_to_add);

    toggleDisable(true);
    await make_post_request(base_url + "/mikai/automations/reset_add_credit", {
