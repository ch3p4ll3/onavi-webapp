function addCredit() {
    ...
    value = localStorage.getItem("add_credit");
    ...
    modal_btn.setAttribute("onclick", "sendAddCredit(this)")

    const myModalAlternative = new bootstrap.Modal('#action-modal', {});
    myModalAlternative.show();
}

async function sendAddCredit(element) {
    let credit_to_add = document.getElementById("modal-text").querySelector("input").value;

    localStorage.setItem("add_credit", credit_to_add);

    toggleDisable(true);
    await make_post_request(base_url + `/mikai/credit/add`, {
